					...
					<?php
						for($i=0;$i<sizeof($subids);$i++)
						{
							...
							if($creds5['status']==0)
							{
								...
								echo(" <option id='incomplete' value = '".$classvalue."'> ".ucwords($subnames[$i]).": ".ucwords($years[$i])." ".ucwords($coursenames[$i])." (Sem ".ucwords($divisions[$i]).") </option> ");
							}else{
								echo(" <option id='complete' value = '".$classvalue."'> ".ucwords($subnames[$i]).": ".ucwords($years[$i])." ".ucwords($coursenames[$i])." (Sem ".ucwords($divisions[$i]).") </option> ");
							}
